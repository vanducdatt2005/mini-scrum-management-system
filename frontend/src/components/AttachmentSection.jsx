...
export default function AttachmentSection({ entityId, entityType = 'task' }) {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      loadAttachments(); // Tải lại danh sách sau khi upload thành công
    } catch (err) {
  ...
  return (
    <div className="mt-6 border-t border-outline-variant/20 pt-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <label className={`cursor-pointer flex items-center gap-1 text-[11px] font-bold px-3 py-1.5 rounded-full bg-primary/10 text-primary hover:bg-primary/20 transition-all ${uploading ? 'opacity-50' : ''}`}>
      ...
      <div className="grid grid-cols-1 gap-2">
        {attachments.map((file) => (
          <div key={file.id} className="flex items-center justify-between p-2 rounded-xl bg-surface-container-low border border-outline-variant/10 hover:border-primary/30 transition-all group">
